...
題材前線は児童向け作品の短い題名を優先し、索引・序文・書誌などを除外、300件に制限します。日本語の反復未分割chunkは同じ自律カリキュラムへ入り、既存の日本語境界・語義学習器をローカルLLMなしで実行します。masteryの予測・因果は監査済みholdoutだけ、概念は汎用抽出後の証拠だけを参照します。
